/*
    ...
    - 서브쿼리 종류에 따라 사버쿼리와 사용하는 연산자가 달라짐 
    
    1. 단일행 서브쿼리(SINGLE ROW SUBQUERY)
    - 서브쿼리의 조회 결과각ㅄ의 개수가 오로지 1개일 때 (한 행 한 열) 
    - 일반 비교연산자 사용 가능 : =, !=, <>, >, <, >=, <=, ... 
*/
-- 노옹철 사원과 같은 부서원들을 조회 
-- 1) 노옹철 사원의 부서코드 조회 
...
SELECT * FROM employee; -- emp_id, emp_name, phone, hire_date
...
SELECT job_code
FROM employee
WHERE job_code = 'J7'; -- J7
-- 박나라 사원 같은 사수 
SELECT manager_id
FROM employee
WHERE manager_id = '207'; -- 207
-- 사번, 이름, 직급코드, 사수사번 

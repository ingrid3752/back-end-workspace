...
SELECT * FROM employee;
SELECT * FROM department; -- JOIN department ON (dept_code = dept_id)
SELECT * FROM location; -- JOIN location ON (location_id = local_code)
SELECT * FROM national; -- JOIN national USING (national_code)
SELECT * FROM job; -- JOIN job USING (job_code)
...
SELECT * FROM national; -- JOIN national USING (national_code)
SELECT * FROM job; -- JOIN job USING (job_code)
SELECT dept_title 부서명, sum(salary) 합계, format(avg(salary), 0) 평균
FROM employee
JOIN department ON (dept_code = dept_id)
GROUP BY dept_title;
CREATE OR REPLACE VIEW vw_groupdept
AS SELECT dept_code 부서코드, sum(salary) 합계, format(avg(salary), 0) 평균
   FROM employee
   JOIN department ON (dept_code = dept_id)
   GROUP BY dept_code;

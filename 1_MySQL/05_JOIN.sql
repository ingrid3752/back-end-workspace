...
SELECT * FROM employee; -- emp_id, emp_name, bonus
SELECT * FROM department; -- dept_id, dept_title
...
-- employee : emp_id, emp_name, salary dept_code, job_code join department,job
-- job : job_name
-- department : dept_title location_id join location
-- location : local_name
...
-- 2. 70년대생 이면서 여자이고, 성이 전 씨인 직원들의 = WHERE
--    직원명, 주민번호, 부서명, 직급명 조회
SELECT emp_name, emp_no, dept_title, job_name
FROM employee
JOIN department ON (dept_code = dept_id)
JOIN job USING (job_code)
WHERE substr(emp_no,1, 7) 
...
SELECT * FROM employee; -- emp_name, bonus, salary * 12, dept_code
SELECT * FROM department; -- dept_id, dept_title, location_id
SELECT * FROM location; -- local_code, local_name

SELECT emp_name, bonus,format((salary + salary * bonus) * 12, 0) 연봉,
...
FROM employee
JOIN job USING (job_code)
WHERE job_code = 'J4' OR 'J7' AND bonus IS NULL;

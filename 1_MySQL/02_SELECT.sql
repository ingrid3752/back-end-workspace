...
SELECT emp_id "사번", emp_name "사원명", hire_date "입사일", ent_yn "재직여부"
FROM employee
WHERE ent_yn = 'N';
...
--    사번, 사원명, 입사일(hire-date), 부서코드 조회 (정렬은 먼저 입사한 기준)
...
SELECT emp_id "사번", emp_name "사원명", salary "급여",(salary + salary * bonus) * 12 연봉
FROM employee

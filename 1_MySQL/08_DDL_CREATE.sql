...
INSERT INTO member 
	VALUES(3, 'user03', 'pass03', '공재욱', '남', NULL,NULL, current_date());
...
SELECT * FROM member;
SELECT * FROM mem_grade;
-- >> 부모 테이블(mem_grade)에서 테이터값을 삭제할 경우 어떤 문제가 발생하는지! 
...
/*
	자식테이블 생성시 외래키 제약조건을 부여할 때 삭제옵션 지정 가능
	 - 삭제옵션 :  부모테이블의 테이터 삭제 시 그 데이터를 사용하고 있는 
			    자식테이블의 값을 어떻게 처리할건지
...
CREATE TABLE employee_copy
AS SELECT emp_id '사번', emp_name '직원명', salary '급여', salary * 12 '연봉'
   FROM kh.employee;
